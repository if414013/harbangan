...
const PROVIDERS_MIXED: ProvidersStatusData = {
  providers: {
    ...
    openai: { connected: true, email: 'user@openai.com' },
  },
}
...
const PROVIDERS_ALL_CONNECTED: ProvidersStatusData = {
  providers: {
    ...
    gemini: { connected: true, email: 'g@google.com' },
...
test.describe('Provider OAuth on Profile page', () => {
  ...
  test.describe('PROVIDERS section structure', () => {
    ...
    test('renders 3 provider cards (anthropic, gemini, openai — no kiro)', async ({ page }) => {
      await navigateTo(page, '/profile')
    ...
    test('renders providers in order: anthropic, gemini, openai', async ({ page }) => {
      await navigateTo(page, '/profile')
      const titles = page.locator('.providers-grid span.card-title')
      await expect(titles.nth(0)).toContainText('anthropic')
      await expect(titles.nth(1)).toContainText('gemini')
      await expect(titles.nth(2)).toContainText('openai')
    })
  ...
  test.describe('Connection status indicators', () => {
    ...
    test('shows NOT CONNECTED badge for disconnected provider', async ({ page }) => {
      await mockProvidersStatus(page)
      await navigateTo(page, '/profile')
      const card = page.locator('div.provider-card').filter({ hasText: '> gemini' })
      await expect(card.locator(Status.err)).toBeVisible()
      await expect(card.locator(Status.err)).toContainText('NOT CONNECTED')
    })

    test('all three providers show NOT CONNECTED when all disconnected', async ({ page }) => {
    ...
    test('all three providers show CONNECTED when all connected', async ({ page }) => {
  ...
  test.describe('Connect flow — relay modal', () => {
    test.beforeEach(async ({ page }) => {
      await mockProvidersStatus(page, PROVIDERS_ALL_DISCONNECTED)
      await page.route('**/_ui/api/providers/gemini/connect', route =>
        route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            relay_script_url: 'https://gw.example.com/_ui/api/providers/gemini/relay-script?token=abc123',
          }),
        })
      )
    })
    ...
    test('relay modal shows provider name in heading', async ({ page }) => {
      ...
      const card = page.locator('div.provider-card').filter({ hasText: '> gemini' })
      await card.locator('button.btn-save', { hasText: '$ connect' }).click()
      await expect(page.locator('.relay-modal h3')).toContainText('connect gemini')
    })
  ...
  test.describe('Connect flow — polling detects connection', () => {
    test('shows success toast when provider becomes connected during polling', async ({ page }) => {
      ...
      await page.route('**/_ui/api/providers/status', route => {
        ...
        // After 2 polls: gemini becomes connected
        const geminiConnected = pollCount > 2
        route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
            providers: {
              anthropic: { connected: false },
              gemini: { connected: geminiConnected, email: geminiConnected ? 'g@google.com' : undefined },
              openai: { connected: false },
            },
          }),
        })
      ...
      await expectToastMessage(page, 'gemini connected', 'success')
